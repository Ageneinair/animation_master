...
def transfer_one(generator, kp_detector, source_image, driving_video, transfer_params):
    ...
    d = driving_video.shape[2]    ### driving_video.Size([1, 3, 26, 128, 128])
    kp_driving = cat_dict([kp_detector(driving_video[:, :, i:(i + 1)]) for i in range(d)], dim=1)
    # print(source_image.shape)  ##torch.Size([1, 3, 1, 128, 256])
    # sys.exit()
    _, _, _, _, l = source_image.shape  #*
    kp_source1 = kp_detector(source_image[:, :, :, :, :int(l/2)])   #*
    kp_source2 = kp_detector(source_image[:, :, :, :, int(l/2):l])  #*
    #### kp_source2['mean'][:, :, :, 1] += 2
    # kp_driving['mean'].shape [1, 26, 10, 2]
    # kp_source['mean'].shape [1, 1, 10, 2]
    # kp_driving['var'].shape [1, 26, 10, 2, 2]
    # kp_source['var'].shape) [1, 1, 10, 2, 2]
    kp_source = {'mean': torch.cat((kp_source1['mean'], kp_source2['mean']), dim=2),
                 'var': torch.cat((kp_source1['var'], kp_source2['var']), dim=2)}

    kp_driving_norm1 = normalize_kp(kp_driving, kp_source1, **transfer_params['normalization_params'])
    kp_driving_norm2 = normalize_kp(kp_driving, kp_source2, **transfer_params['normalization_params'])
    kp_driving_norm2['mean'][:, :, :, 1] += 2
    # print(kp_driving_norm2['mean'])
    # sys.exit()
    kp_driving_norm = {'mean': torch.cat((kp_driving_norm1['mean'], kp_driving_norm2['mean']), dim=2),
                       'var': torch.cat((kp_driving_norm1['var'], kp_driving_norm2['var']), dim=2)}
    # print(kp_driving_norm['var'].shape)
    # sys.exit()
